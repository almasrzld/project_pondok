<aside
    ...
    <nav class="p-4 space-y-2 text-sm">
        ...
        <div
            x-data="{ open: {{ request()->routeIs('dashboard.rapot*') || request()->routeIs('dashboard.semester*') ? 'true' : 'false' }} }"
        >
            <button
                @click="open = !open"
                class="w-full flex items-center justify-between px-3 py-2 rounded
                {{ request()->routeIs('dashboard.rapot*') || request()->routeIs('dashboard.semester*')
                    ? 'bg-indigo-50 text-indigo-600 font-semibold'
                    : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
                <span>Rapot</span>
                <svg class="w-4 h-4 transform transition-transform"
            ...
            <div x-show="open" x-collapse class="mt-1 ml-4 space-y-1">
                ...
                <a href="{{ route('dashboard.semester.index') }}"
                   class="block px-3 py-2 rounded text-sm
                   {{ request()->routeIs('dashboard.semester.*')
                        ? 'bg-indigo-100 text-indigo-600 font-semibold'
                        : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
                    Semester
                </a>
